...
def get_db_connection():
    if 'db' not in g:
        #"Kaunastest"
        pass_word = os.environ.get("MYSQLPASS")
        #"loui.mysql.pythonanywhere-services.com"
        hostname = os.environ.get("MYSQLHOST")
        username = os.environ.get("MYSQLUSER")
        g.db = mysql.connector.connect(database="loui$default", user=username, password=pass_word, host=hostname, port=3306)
    return g.db
...
def all_symbols():
    ...
    q.id,
    q.quote_name,
    COALESCE(MAX(CASE WHEN qp.rn = 1 THEN qp.close_price END), 0) AS close_price,
    COALESCE(DATE_FORMAT(
        MAX(CASE WHEN qp.rn = 1 THEN qp.last_date_at END),
        '%Y-%m-%d'
    ), '') AS last_date_at,
    COALESCE(MAX(CASE WHEN qp.rn = 2 THEN qp.close_price END), 0) AS prev_to_last_close_price,
    COALESCE(DATE_FORMAT(
        MAX(CASE WHEN qp.rn = 2 THEN qp.last_date_at END),
        '%Y-%m-%d'
    ), '') AS prev_to_last_date_at
    FROM quotes q
    LEFT JOIN (
        SELECT *,
            ROW_NUMBER() OVER (
